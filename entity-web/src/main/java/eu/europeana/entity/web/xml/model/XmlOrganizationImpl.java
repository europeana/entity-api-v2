...
@JsonPropertyOrder({XmlConstants.XML_FOAF_DEPICTION, XmlConstants.XML_SKOS_PREF_LABEL,
    XmlConstants.XML_EDM_ACRONYM, XmlConstants.XML_SKOS_ALT_LABEL, XmlConstants.XML_DC_DESCRIPTION,
    XmlConstants.XML_FOAF_LOGO, XmlConstants.XML_EDM_EUROPEANA_ROLE, XmlConstants.XML_EDM_COUNTRY,
    XmlConstants.XML_FOAF_HOMEPAGE, XmlConstants.XML_FOAF_PHONE, XmlConstants.XML_FOAF_MBOX,
    XmlConstants.XML_VCARD_HAS_ADDRESS, XmlConstants.XML_VCARD_ADDRESS,
    XmlConstants.XML_DC_IDENTIFIER, XmlConstants.XML_OWL_SAME_AS,
    XmlConstants.XML_EDM_AGGREGATED_VIA, XmlConstants.XML_ORE_IS_AGGREGATED_BY})
public class XmlOrganizationImpl extends XmlBaseEntityImpl {
  ...
  public List<RdfResource> getEuropeanaRole() {
    if (getOrganization().getEuropeanaRole() == null) {
      return null;
    }

    List<RdfResource> roles = new ArrayList<>(getOrganization().getEuropeanaRole().size());
    for (String role : getOrganization().getEuropeanaRole()) {
  ...
  /*
   * ElementWrapper works only on lists and maps. This conversion is needed, because we have a two
   * level object <vcard:hasAddress><vcard:Address>....</vcard:hasAddress></vcard:Address>
   */
